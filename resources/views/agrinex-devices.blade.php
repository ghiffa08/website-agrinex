            ...
            <div class="flex-1 min-w-0 flex flex-col min-h-screen overflow-x-hidden">
                ...
                <main class="flex-1 px-4 md:px-6 xl:px-8 pt-6 pb-28 md:pb-10 w-full max-w-[1400px] mx-auto space-y-6 md:space-y-8">
                    ...
                    <div class="bg-neuBg rounded-[2.5rem] p-6 md:p-8 shadow-[8px_8px_16px_#a3b1c6,-8px_-8px_16px_#ffffff]">
                        ...
                        {{-- Page Header & Refresh --}}
                        <div class="flex justify-between items-start md:items-center mb-8 border-b border-[#a3b1c6]/30 pb-6">
                            <div>
                                <h2 class="text-xl md:text-2xl font-extrabold tracking-tight text-darkText">Semua Perangkat</h2>
                                <p class="text-lightText text-xs md:text-sm mt-1">Daftar semua sensor node AgriNex yang terhubung</p>
                            </div>
                            <button @click="fetchDevices" class="p-3 mt-2 md:mt-0 rounded-xl bg-neuBg shadow-[4px_4px_8px_#a3b1c6,-4px_-4px_8px_#ffffff] text-brand hover:shadow-[inset_4px_4px_8px_#a3b1c6,inset_-4px_-4px_8px_#ffffff] transition-all flex-shrink-0">
                        ...
                        <div x-show="!loadingDevices" class="space-y-8">
                            <template x-for="(group, area) in groupedDevices" :key="area">
                                <div>
                                    <h3 class="text-sm md:text-base font-bold text-lightText uppercase tracking-wider mb-4 px-2" x-text="area || 'Unassigned'"></h3>
                                            ...
                                            <div class="bg-neuBg rounded-[2rem] p-5 shadow-[8px_8px_16px_#a3b1c6,-8px_-8px_16px_#ffffff] flex flex-col gap-6">
                                                ...
                                                <div class="flex justify-between items-center">
                                                    <div class="flex items-center gap-2">
                                                        ...
                                                        <span class="font-bold text-darkText text-base md:text-lg" x-text="node.name"></span>
                                                    </div>
                                                ...
                                                <div class="grid grid-cols-3 gap-3">
                                                    <!-- LEMBAP -->
                                                    <div class="bg-neuBg rounded-2xl p-3 shadow-[inset_4px_4px_8px_#a3b1c6,inset_-4px_-4px_8px_#ffffff] flex flex-col items-center justify-center text-center">
                                                        <span class="text-[9px] font-bold text-lightText uppercase mb-1">Lembap</span>
                                                        <div class="flex items-baseline gap-1">
                                                            <span class="text-lg md:text-xl font-extrabold" :class="(node.soil_moisture_pct ?? 0) < 30 ? 'text-[#EF4444]' : 'text-[#00D26A]'" x-text="node.soil_moisture_pct ?? '--'"></span>
                                                            <span class="text-[10px] md:text-xs font-bold text-lightText">%</span>
                                                        </div>
                                                    </div>
                                                    <!-- SUHU -->
                                                    <div class="bg-neuBg rounded-2xl p-3 shadow-[inset_4px_4px_8px_#a3b1c6,inset_-4px_-4px_8px_#ffffff] flex flex-col items-center justify-center text-center">
                                                        ...
                                                        <div class="flex items-baseline gap-1">
                                                            <span class="text-lg md:text-xl font-extrabold text-darkText" x-text="node.temperature_c ?? '--'"></span>
                                                            <span class="text-[10px] md:text-xs font-bold text-lightText">°C</span>
                                                        </div>
                                                    </div>
                                                    <!-- VOLT -->
                                                    <div class="bg-neuBg rounded-2xl p-3 shadow-[inset_4px_4px_8px_#a3b1c6,inset_-4px_-4px_8px_#ffffff] flex flex-col items-center justify-center text-center">
                                                        ...
                                                        <div class="flex items-baseline gap-1">
                                                            <span class="text-lg md:text-xl font-extrabold" :class="(node.battery_voltage_v ?? 0) < 3.2 ? 'text-amber-500' : 'text-darkText'" x-text="node.battery_voltage_v ?? '--'"></span>
                                                            <span class="text-[10px] md:text-xs font-bold text-lightText">V</span>
                                                        </div>
                                                    </div>
                                                </div>
